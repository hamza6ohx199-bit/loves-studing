...
class TasksEngine {
  ...
  renderTaskList() {
    ...
    if (this.tasks.length === 0) {
      listEl.innerHTML = '<div style="text-align:center; padding:30px; color:var(--text-muted);">لا توجد مهام نشطة حالياً! أضف مهمة جديدة من الأعلى ⚔️</div>';
      return;
    }
    ...
    this.tasks.forEach(task => {
      ...
      const tagClass = `tag-${task.subject || 'science'}`;
      const subjectText = subjectLabels[task.subject] || task.subject;
      ...
      item.innerHTML = `
        <div class="task-left">
          ...
          <div>
            ...
            <span class="task-tag ${tagClass}">${subjectText}</span>
          </div>
        </div>
        <div class="task-right-actions">
          <div class="task-reward">+${task.xp} XP | 🪙 ${task.gold}</div>
          <button style="background:none; border:none; color:var(--accent-ruby); cursor:pointer; font-size:16px;" onclick="window.tasksEngine.deleteTask('${task.id}')" title="حذف المهمة">🗑️</button>
        </div>
      `;
