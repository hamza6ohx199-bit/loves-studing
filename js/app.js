...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const questInput = document.getElementById('new-quest-input');
  
  const handleAddQuest = () => {
    const selectSubject = document.getElementById('new-quest-subject');
    const selectDifficulty = document.getElementById('new-quest-difficulty');
    if (questInput && questInput.value.trim()) {
      const subject = selectSubject ? selectSubject.value : 'science';
      const difficulty = selectDifficulty ? selectDifficulty.value : 'medium';
      window.tasksEngine.addTask(questInput.value.trim(), subject, difficulty);
      questInput.value = '';
    }
  };
  ...
  if (window.rewardsEngine) window.rewardsEngine.renderVisaCard();
  if (window.aiQuizEngine) window.aiQuizEngine.updateApiKeyStatus();
});
